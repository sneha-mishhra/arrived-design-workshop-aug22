...
export function Navbar({ nav, ctaText, ctaHref }: NavbarProps) {
  ...
  return (
    <div className="hidden items-center gap-3 md:flex">
      <nav className="flex items-center gap-1 rounded-full border border-black/10 bg-white/80 p-1 text-[11px] uppercase tracking-[0.1em] shadow-[0_2px_14px_rgba(9,9,9,0.07)] backdrop-blur-sm">
        {nav.map((link) => {
          ...
          return (
            <ScrollLink
              ...
              className={`flex items-center rounded-full px-3.5 py-1.5 transition-colors ${
                isPrimary
                  ? "bg-[#171310] text-white hover:opacity-90"
                  : isActive
                    ? "bg-brand-violet text-white"
                    : "text-[#171310]/70 hover:bg-black/5 hover:text-[#171310]"
              }`}
            >
              {link.label}
            </ScrollLink>
